...
class CubicFilter(nn.Module):
    def __init__(self, num_in_channels=64, num_out_channels=64, batch_size=1):
        ...
        self.fc_cubic = nn.Linear(num_out_channels, 60)  # cubic
    ...
    def get_cubic_mask(self, feat, img):
        #######################################################
        ####################### Cubic #########################
        feat_cubic = torch.cat((feat, img), dim=1)
        ...
        x_axis = torch.arange(img.shape[2]).view(-1, 1).repeat(1, img.shape[3]) / img.shape[2]
        y_axis = torch.arange(img.shape[3]).repeat(img.shape[2], 1) / img.shape[3]
        x_axis = x_axis.to(img.device)
        y_axis = y_axis.to(img.device)
        ...
        cubic_mask[0, 0, :, :] = (
            R[0, 0] * (x_axis ** 3)
            + R[0, 1] * (x_axis ** 2) * y_axis
            + R[0, 2] * (x_axis ** 2) * img[0, 0, :, :]
            + R[0, 3] * (x_axis ** 2)
            + R[0, 4] * x_axis * (y_axis ** 2)
            + R[0, 5] * x_axis * y_axis * img[0, 0, :, :]
            + R[0, 6] * x_axis * y_axis
            + R[0, 7] * x_axis * (img[0, 0, :, :] ** 2)
            + R[0, 8] * x_axis * img[0, 0, :, :]
            + R[0, 9] * x_axis
            + R[0, 10] * (y_axis ** 3)
            + R[0, 11] * (y_axis ** 2) * img[0, 0, :, :]
            + R[0, 12] * (y_axis ** 2)
            + R[0, 13] * y_axis * (img[0, 0, :, :] ** 2)
            + R[0, 14] * y_axis * img[0, 0, :, :]
            + R[0, 15] * y_axis
            + R[0, 16] * (img[0, 0, :, :] ** 3)
            + R[0, 17] * (img[0, 0, :, :] ** 2)
            + R[0, 18] * img[0, 0, :, :]
        ...
        cubic_mask[0, 1, :, :] = (
            R[0, 20] * (x_axis ** 3)
            + R[0, 21] * (x_axis ** 2) * y_axis
            + R[0, 22] * (x_axis ** 2) * img[0, 1, :, :]
            + R[0, 23] * (x_axis ** 2)
            + R[0, 24] * x_axis * (y_axis ** 2)
            + R[0, 25] * x_axis * y_axis * img[0, 1, :, :]
            + R[0, 26] * x_axis * y_axis
            + R[0, 27] * x_axis * (img[0, 1, :, :] ** 2)
            + R[0, 28] * x_axis * img[0, 1, :, :]
            + R[0, 29] * x_axis
            + R[0, 30] * (y_axis ** 3)
            + R[0, 31] * (y_axis ** 2) * img[0, 1, :, :]
            + R[0, 32] * (y_axis ** 2)
            + R[0, 33] * y_axis * (img[0, 1, :, :] ** 2)
            + R[0, 34] * y_axis * img[0, 1, :, :]
            + R[0, 35] * y_axis
            + R[0, 36] * (img[0, 1, :, :] ** 3)
            + R[0, 37] * (img[0, 1, :, :] ** 2)
            + R[0, 38] * img[0, 1, :, :]
        ...
        cubic_mask[0, 2, :, :] = (
            R[0, 40] * (x_axis ** 3)
            + R[0, 41] * (x_axis ** 2) * y_axis
            + R[0, 42] * (x_axis ** 2) * img[0, 2, :, :]
            + R[0, 43] * (x_axis ** 2)
            + R[0, 44] * x_axis * (y_axis ** 2)
            + R[0, 45] * x_axis * y_axis * img[0, 2, :, :]
            + R[0, 46] * x_axis * y_axis
            + R[0, 47] * x_axis * (img[0, 2, :, :] ** 2)
            + R[0, 48] * x_axis * img[0, 2, :, :]
            + R[0, 49] * x_axis
            + R[0, 50] * (y_axis ** 3)
            + R[0, 51] * (y_axis ** 2) * img[0, 2, :, :]
            + R[0, 52] * (y_axis ** 2)
            + R[0, 53] * y_axis * (img[0, 2, :, :] ** 2)
            + R[0, 54] * y_axis * img[0, 2, :, :]
            + R[0, 55] * y_axis
            + R[0, 56] * (img[0, 2, :, :] ** 3)
            + R[0, 57] * (img[0, 2, :, :] ** 2)
            + R[0, 58] * img[0, 2, :, :]
        ...
        img_cubic = torch.clamp(img + cubic_mask, 0.0, 1.0)
        return img_cubic
...
class GraduatedFilter(nn.Module):
    def __init__(self, num_in_channels=64, num_out_channels=64):
        ...
        self.fc_graduated = nn.Linear(num_out_channels, 24)
        self.upsample = nn.Upsample(size=(300, 300), mode="bilinear", align_corners=False)
        self.dropout = nn.Dropout(0.5)
    ...
    def get_inverted_mask(self, factor, invert, d1, d2, max_scale: float, top_line):
        # factor -- tensor(0., device='cuda:0') -- 
        # invert -- tensor(1., device='cuda:0')

        # if (invert == 1).all():
        if torch.abs(invert - 1.0).mean() < 0.01:
            # if (factor >= 1).all():
            if factor.min() >= 1.0:
                diff = ((factor - 1)) / 2.0 + 1.0
                grad1 = (diff - factor) / d1
                grad2 = (1 - diff) / d2
                mask_scale = torch.clamp(factor + grad1 * top_line + grad2 * top_line, min=1.0, max=max_scale)
            else:
                diff = ((1 - factor)) / 2.0 + factor
                grad1 = (diff - factor) / d1
                grad2 = (1 - diff) / d2
                mask_scale = torch.clamp(factor + grad1 * top_line + grad2 * top_line, min=0.0, max=1.0)
        else:
            # if (factor >= 1).all():
            if factor.min() >= 1.0:
                diff = ((factor - 1)) / 2.0 + 1.0
                ...
                mask_scale = torch.clamp(1 + grad1 * top_line + grad2 * top_line, min=1.0, max=max_scale)
            else:
                diff = ((1 - factor)) / 2 + factor
                grad1 = (diff - 1) / d1
                grad2 = (factor - diff) / d2
                mask_scale = torch.clamp(1 + grad1 * top_line + grad2 * top_line, min=0.0, max=1.0)

        mask_scale = torch.clamp(mask_scale.unsqueeze(0), 0.0, max_scale)
        return mask_scale
    ...
    def get_graduated_mask(self, feat, img):
        ...
        y_axis_dist4 = torch.clamp(self.tanh01(G[0, 12]), 0.0, y_axis_dist1.data)
        y_axis_dist5 = torch.clamp(self.tanh01(G[0, 13]), 0.0, y_axis_dist2.data)
        y_axis_dist6 = torch.clamp(self.tanh01(G[0, 14]), 0.0, y_axis_dist3.data)
        ...
        mask_scale_1 = torch.clamp(mask_scale_1.unsqueeze(0), 0.0, max_scale)
        ...
        mask_scale_4 = torch.clamp(mask_scale_4.unsqueeze(0), 0.0, max_scale)
        ...
        mask_scale_7 = torch.clamp(mask_scale_7.unsqueeze(0), 0.0, max_scale)

        mask_scale = torch.clamp(mask_scale_1 * mask_scale_4 * mask_scale_7, 0.0, max_scale)
...
class EllipticalFilter(nn.Module):
    def __init__(self, num_in_channels=64, num_out_channels=64):
        ...
        self.fc_elliptical = nn.Linear(num_out_channels, 24)  # elliptical
        self.upsample = nn.Upsample(size=(300, 300), mode="bilinear", align_corners=False)
        self.dropout = nn.Dropout(0.5)
    ...
    def where(self, cond, x_1, x_2: float):
        cond = cond.float()
        return (cond * x_1) + ((1 - cond) * x_2)
    ...
    def get_mask(
        ...
        mask_scale = self.where(
            ellipse_equation_part1 + ellipse_equation_part2 < 1,
            (torch.sqrt((x_axis - shift_x) ** 2 + (y_axis - shift_y) ** 2 + eps) * (1 - scale_factor)) / radius
            + scale_factor,
            1.0,
        )
    ...
    def get_elliptical_mask(self, feat, img):
        ...
        angle_1 = (
            torch.acos(
                torch.clamp(
                    (y_axis - y_coord1) / (torch.sqrt((x_axis - x_coord1) ** 2 + (y_axis - y_coord1) ** 2 + eps1)),
                    -1 + eps2,
                    1 - eps2,
                )
            )
        ...
        angle_2 = (
            torch.acos(
                torch.clamp(
                    (y_axis - y_coord2) / (torch.sqrt((x_axis - x_coord2) ** 2 + (y_axis - y_coord2) ** 2 + eps1)),
                    -1 + eps2,
                    1 - eps2,
                )
            )
        ...
        angle_3 = (
            torch.acos(
                torch.clamp(
                    (y_axis - y_coord3) / (torch.sqrt((x_axis - x_coord3) ** 2 + (y_axis - y_coord3) ** 2 + eps1)),
                    -1 + eps2,
                    1 - eps2,
                )
            )
        ...
        radius_1 = (a1 * b1) / torch.sqrt(
            (a1 ** 2) * (torch.sin(angle_1) ** 2) + (b1 ** 2) * (torch.cos(angle_1) ** 2) + eps1
        )

        radius_2 = (a2 * b2) / torch.sqrt(
            (a2 ** 2) * (torch.sin(angle_2) ** 2) + (b2 ** 2) * (torch.cos(angle_2) ** 2) + eps1
        )

        radius_3 = (a3 * b3) / torch.sqrt(
            (a3 ** 2) * (torch.sin(angle_3) ** 2) + (b3 ** 2) * (torch.cos(angle_3) ** 2) + eps1
        )
        ...
        mask_scale_1 = torch.cat((mask_scale1, mask_scale2, mask_scale3), dim=0)
        mask_scale_1_rad = torch.clamp(mask_scale_1.unsqueeze(0), 0.0, max_scale)
        ...
        mask_scale_4 = torch.cat((mask_scale4, mask_scale5, mask_scale6), dim=0)
        mask_scale_4_rad = torch.clamp(mask_scale_4.unsqueeze(0), 0.0, max_scale)
        ...
        mask_scale_7 = torch.cat((mask_scale7, mask_scale8, mask_scale9), dim=0)
        mask_scale_7_rad = torch.clamp(mask_scale_7.unsqueeze(0), 0.0, max_scale)

        ############ Mix the ellipses together by multiplication
        mask_scale_elliptical = torch.clamp(mask_scale_1_rad * mask_scale_4_rad * mask_scale_7_rad, 0.0, max_scale)
...
class DeepLPFParameterPrediction(nn.Module):
    ...
    def forward(self, x):
        ...
        mask_scale_fuse = torch.clamp(mask_scale_graduated + mask_scale_elliptical, 0.0, 2.0)

        # img_fuse = torch.clamp(img_cubic * mask_scale_fuse, 0.0, 1.0)
        img_cubic *= mask_scale_fuse
        img_fuse = torch.clamp(img_cubic, 0.0, 1.0)

        img = torch.clamp(img_fuse + img, 0.0, 1.0)
