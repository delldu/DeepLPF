...
def get_model():
    ...
    model_path = "models/image_deeplpf.pth"
    cdir = os.path.dirname(__file__)
    checkpoint = model_path if cdir == "" else cdir + "/" + model_path

    model = deeplpf.DeepLPFNet()
    ...
    # if not os.path.exists("output/image_deeplpf.torch"):
    #     model.save("output/image_deeplpf.torch")
...
def image_client(name, input_files, output_dir):
    ...
    for filename in image_filenames:
        ...
        context = cmd.deeplpf(filename, output_file)
...
def image_server(name, host="localhost", port=6379):
    ...
    return redos.image.service(name, "image_deeplpf", do_service, host, port)
...
def video_service(input_file, output_file, targ):
    ...
    print(f"  deeplpf {input_file}, save to {output_file} ...")
...
def video_client(name, input_file, output_file):
    ...
    context = cmd.deeplpf(input_file, output_file)
    redo = redos.Redos(name)
    redo.set_queue_task(context)
